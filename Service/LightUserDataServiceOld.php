 ...
 */
class LightUserDataServiceOld implements PluginInstallerInterface, PluginPostInstallerInterface
{
    ...
    public function list(string $directory = null): array
    {
        ...
        if (null !== $directory) {
            $where->and()->key("dir")->equals($directory);
        }

        $api = $this->getFactory()->getResourceApi();
        $resourceIds = $api->fetchAll([
            Columns::inst()->set(["resource_identifier"])->singleColumn(),
            $where,
        ]);

        foreach ($resourceIds as $id) {
            $ret[] = $this->getResourceInfoByResourceIdentifier($id);
        }
...
//        az($path, $options);
        ...
        if (true === $isAddOperation) {
            ...
            // check that the dir/filename combo doesn't already exist for that user
            if (true === $this->hasEntryByDirFilename($row['dir'], $row['filename'], $user->getId())) {
                $this->error("A resource already exists with the path: " . $row['dir'] . "/" . $row['filename']);
            }
        ...
        //--------------------------------------------
            ...
            // check that the dir/filename combo doesn't already exist for that user, in case of a change
            if (
                $resource['dir'] !== $row['dir'] ||
                $resource['filename'] !== $row['filename']
            ) {
                if (true === $this->hasEntryByDirFilename($row['dir'], $row['filename'], $user->getId())) {
                    $this->error("A resource already exists with the path: " . $row['dir'] . "/" . $row['filename']);
                }
...
//                    $this->factory->getTagApi()->removeUnusedTags(); // shall we?
    ...
    {
//        $file = $this->getUserDir() . "/files/$path"; // ?
     ...
     *
     * @param string $resourceId
     * @param LightWebsiteUser|null|false $user
     * @return string
     * @throws \Exception
     */
    public function getContentByResourceId(string $resourceId, LightWebsiteUser $user = null): string
    {
        ...
        $info = $this->getResourceInfoByResourceIdentifier($resourceId);
        $path = $info['abs_path'];
        if (false === file_exists($path)) {
            $this->error("File doesn't exist with resourceId: $resourceId, path=\"$path\".");
        }
...
//    }
     ...
     * @throws \Exception
     */
    private function hasEntryByDirFilename(string $dir, string $filename, int $userId): bool
    {
        $res = $this->getFactory()->getResourceApi()->fetch([
            Where::inst()->key("lud_user_id")->equals($userId)
                ->and()->key("dir")->equals($dir)
                ->and()->key("filename")->equals($filename)
        ]);
        return (false !== $res);
    }
